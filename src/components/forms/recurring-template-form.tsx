...
type LocalRecurringLineItemValue = RecurringTemplateFormLineItemValue & {
  localId: string;
};
...
function emptyLineItem(): RecurringTemplateFormLineItemValue {
  return {
    description: "",
    quantity: "1",
    unitPrice: "0",
...
function stripLocalId(item: LocalRecurringLineItemValue): RecurringTemplateFormLineItemValue {
  return {
...
export function RecurringTemplateForm({ clients }: RecurringTemplateFormProps) {
  ...
  const [lineItems, setLineItems] = useState<LocalRecurringLineItemValue[]>(() =>
    state.values.lineItems.map((item) => ({
      ...item,
      localId: `recurring-line-item-${lineItemIdRef.current++}`,
    })),
  );
  ...
  function updateLineItem(localId: string, field: keyof RecurringTemplateFormLineItemValue, value: string) {
  ...
  return (
    <form action={formAction} className="grid gap-6">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <label className="grid gap-2 text-sm text-slate-200">
          ...
          <select
            ...
            onChange={(event) =>
              setValues((current) => ({
                ...current,
                frequency: event.target.value === "WEEKLY" ? "WEEKLY" : "MONTHLY",
              }))
            }
            className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-base text-white outline-none transition focus:border-cyan-300/50"
          >
        ...
        <TextInput
          label={isMonthly ? "Repeat every X months" : "Repeat every X weeks"}
          name="intervalCount"
          type="number"
        ...
        <TextInput
          label={isMonthly ? "First invoice date (1st-28th)" : "First invoice date"}
          name="startsAt"
          type="date"
        ...
        <TextInput
          ...
          errors={state.fieldErrors.dueInDays}
          min={0}
          step={1}
          required
        />
        ...
        <label className="grid gap-2 text-sm text-slate-200">
          ...
          <select
            ...
            onChange={(event) =>
              setValues((current) => ({
                ...current,
                invoiceStatus: event.target.value === "SENT" ? "SENT" : "DRAFT",
              }))
            }
            className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-base text-white outline-none transition focus:border-cyan-300/50"
          >
          ...
          <FieldError errors={state.fieldErrors.invoiceStatus} />
        </label>
      ...
      <label className="grid gap-2 text-sm text-slate-200">
        ...
        <textarea
          ...
          placeholder="Internal notes for operators or invoice footer guidance."
          className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-base text-white outline-none transition focus:border-cyan-300/50"
        />
      ...
      <section className="grid gap-4 rounded-[1.75rem] border border-white/8 bg-white/[0.03] p-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-white">Recurring line items</h2>
            <p className="mt-1 text-sm text-slate-400">
              Keep template pricing in minor units so generated invoices match V1 invoice math exactly.
            </p>
          </div>
        ...
        {lineItems.map((item, index) => (
          ...
          >
            <TextInput
              ...
              name={`recurringLineDescription-${index}`}
            ...
            <TextInput
              label="Quantity"
              name={`recurringLineQuantity-${index}`}
              type="number"
              value={item.quantity}
            ...
            <TextInput
              ...
              name={`recurringLineUnitPrice-${index}`}
              type="number"
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {pending ? "Saving template..." : "Save recurring template"}
        </button>
